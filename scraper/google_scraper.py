...
# ============================================================
# HireSense — Google Internship Scraper (Phase 1)
...
def scrape_google_postings():
    ...
    print(f"\nTotal Google postings found: {len(google_postings)}")
    active = sum(1 for p in google_postings if p["is_active"])
    print(f"Active: {active} | Closed: {len(google_postings) - active}")
    return google_postings


def get_or_create_company():
    print("\nChecking companies table...")
    existing = supabase.table("companies").select("id").eq("name", "Google").execute()

    if existing.data:
        company_id = existing.data[0]["id"]
        print(f"Google already exists (id: {company_id})")
        return company_id

    result = supabase.table("companies").insert({
        "name": "Google",
        "description": "Multinational technology company focused on search, cloud, and AI.",
        "industry": "Technology",
    ...
    print(f"Inserted Google (id: {company_id})")
    return company_id
...
if __name__ == "__main__":
    postings = scrape_google_postings()
    if postings:
        company_id = get_or_create_company()
        insert_postings(postings, company_id)
    else:
        print("No Google postings found.")