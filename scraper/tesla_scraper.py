...
# ============================================================
# HireSense — Tesla Internship Scraper (Phase 1)
...
def scrape_tesla_postings():
    ...
    print(f"\nTotal Tesla postings found: {len(tesla_postings)}")
    active = sum(1 for p in tesla_postings if p["is_active"])
    print(f"Active: {active} | Closed: {len(tesla_postings) - active}")
    return tesla_postings


def get_or_create_company():
    print("\nChecking companies table...")
    existing = supabase.table("companies").select("id").eq("name", "Tesla").execute()

    if existing.data:
        company_id = existing.data[0]["id"]
        print(f"Tesla already exists (id: {company_id})")
        return company_id

    result = supabase.table("companies").insert({
        "name": "Tesla",
        "description": "Electric vehicle and clean energy company focused on accelerating the world's transition to sustainable energy.",
    ...
    print(f"Inserted Tesla (id: {company_id})")
    return company_id
...
if __name__ == "__main__":
    postings = scrape_tesla_postings()
    if postings:
        company_id = get_or_create_company()
        insert_postings(postings, company_id)
    else:
        print("No Tesla postings found.")