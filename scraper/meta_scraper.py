...
# ============================================================
# HireSense — Meta Internship Scraper (Phase 1)
...
def scrape_meta_postings():
    ...
    for listing in listings:
        ...
        # Filter for Meta only
        if "meta" not in company.lower():
            continue
    ...
    print(f"\nTotal Meta postings found: {len(meta_postings)}")
    active = sum(1 for p in meta_postings if p["is_active"])
    print(f"Active: {active} | Closed: {len(meta_postings) - active}")
    return meta_postings


def get_or_create_company():
    print("\nChecking companies table...")
    existing = supabase.table("companies").select("id").eq("name", "Meta").execute()

    if existing.data:
        company_id = existing.data[0]["id"]
        print(f"Meta already exists (id: {company_id})")
        return company_id

    result = supabase.table("companies").insert({
        "name": "Meta",
        "description": "Multinational technology company focused on social media, AR/VR, and AI.",
        "industry": "Technology",
    ...
    print(f"Inserted Meta (id: {company_id})")
    return company_id
...
if __name__ == "__main__":
    postings = scrape_meta_postings()
    if postings:
        company_id = get_or_create_company()
        insert_postings(postings, company_id)
    else:
        print("No Meta postings found.")