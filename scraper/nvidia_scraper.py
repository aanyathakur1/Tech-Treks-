...
# ============================================================
# HireSense — NVIDIA Internship Scraper (Phase 1)
...
def scrape_nvidia_postings():
    ...
    for listing in listings:
        ...
        if "nvidia" not in company.lower():
            continue
    ...
    print(f"\nTotal NVIDIA postings found: {len(nvidia_postings)}")
    active = sum(1 for p in nvidia_postings if p["is_active"])
    print(f"Active: {active} | Closed: {len(nvidia_postings) - active}")
    return nvidia_postings


def get_or_create_company():
    print("\nChecking companies table...")
    existing = supabase.table("companies").select("id").eq("name", "NVIDIA").execute()

    if existing.data:
        company_id = existing.data[0]["id"]
        print(f"NVIDIA already exists (id: {company_id})")
        return company_id

    result = supabase.table("companies").insert({
        "name": "NVIDIA",
        "description": "Multinational technology company designing GPUs and system-on-chip units, leading AI and deep learning hardware.",
    ...
    print(f"Inserted NVIDIA (id: {company_id})")
    return company_id
...
if __name__ == "__main__":
    postings = scrape_nvidia_postings()
    if postings:
        company_id = get_or_create_company()
        insert_postings(postings, company_id)
    else:
        print("No NVIDIA postings found.")