...
# ============================================================
# HireSense — Netflix Internship Scraper (Phase 1)
...
def scrape_netflix_postings():
    ...
    for listing in listings:
        ...
        # Filter for Netflix only
        if "netflix" not in company.lower():
            continue
    ...
    print(f"\nTotal Netflix postings found: {len(netflix_postings)}")
    active = sum(1 for p in netflix_postings if p["is_active"])
    print(f"Active: {active} | Closed: {len(netflix_postings) - active}")
    return netflix_postings


def get_or_create_company():
    print("\nChecking companies table...")
    existing = supabase.table("companies").select("id").eq("name", "Netflix").execute()

    if existing.data:
        company_id = existing.data[0]["id"]
        print(f"Netflix already exists (id: {company_id})")
        return company_id

    result = supabase.table("companies").insert({
        "name": "Netflix",
        "description": "Global streaming entertainment service with over 260 million paid memberships in 190+ countries.",
    ...
    print(f"Inserted Netflix (id: {company_id})")
    return company_id
...
if __name__ == "__main__":
    postings = scrape_netflix_postings()
    if postings:
        company_id = get_or_create_company()
        insert_postings(postings, company_id)
    else:
        print("No Netflix postings found.")